...
var
  DirectoryRoot, DirectoryErrors, DirectoryTFOMSErrors, DirectoryProcessed, DirectoryOutput, DirectoryInvoice, DirectoryInvoiceMTR: string;
    ...
    For i := 0 to High(arrayResults) do
      begin
        case arrayResults[i] of
          ...
          23 : arrayResultsD[i] := 'Одна или несколько подписей некорректна или нет доверия. Возможно необходимо обновить корневой сертификат и перезагрузить компьютер';
...
begin
  ...
  if MatchesMask(inputFileName, 'SH_*_*_*.zip') or
     ...
     MatchesMask(inputFileName, 'MSMP_*_МТР_*.zip') then //MTP – по-русски
    begin
      Result := True;
    end;
