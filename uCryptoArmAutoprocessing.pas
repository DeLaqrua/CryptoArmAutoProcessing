...
type
  TVerInfo = packed record
    ...
    Minor, Major, Build, Release: word; //Release – количество дней, начиная с 1 января 2000 года (так по умолчанию в Delphi, по идее нужно считать дни с даты Релиза программы)
                                        //Build – количество секунд, начиная с 00:00, делённое на 2
  end;
var
  s: TResourceStream;
  v: TVerInfo;
...
begin
  ...
  try
    ...
    if System.SysUtils.DirectoryExists(DirectoryInvoice) = False then
      ShowMessage('Проверьте путь к директории со Счетами. Папки не существует')
    else
    if System.SysUtils.DirectoryExists(DirectoryInvoiceMTR) = False then
      ShowMessage('Проверьте путь к директории со Счетами-МТР. Папки не существует')
    else
    if System.SysUtils.DirectoryExists(DirectoryAct) = False then
      ShowMessage('Проверьте путь к директории с Актами. Папки не существует')
    else
    if System.SysUtils.DirectoryExists(DirectoryActMTR) = False then
      ShowMessage('Проверьте путь к директории с Актами-МТР. Папки не существует')
    else
...
begin
  ...
  try
    ...
    AddLog(DateToStr(Now) + ' ' + TimeToStr(Now) + '  Начало проверки подписей из архива: "' + inputOriginalArchiveFileName + '". Время проверки одной подписи длится до 30 секунд. Ожидайте...' + #13#10, isInformation);
    ...
    begin
      ...
      begin
        //Формируем протокол в папку со Счетами
      ...
      begin
        //Формируем протокол в папку со Счетами-МТР
      ...
      begin
        //Формируем протокол в папку с Актами
      ...
      begin
        //Формируем протокол в папку с Актами-МТР
...
begin
  ...
  if MatchesMask(inputArchiveFileName, 'CON*') or //MTP – по-русски
     MatchesMask(inputArchiveFileName, 'RCON*') then
    InvoiceType := REGULAR_ACT;

  if MatchesMask(inputArchiveFileName, 'CON_*_МТР*') or //MTP – по-русски
     MatchesMask(inputArchiveFileName, 'CON_*_MTP*') or //MTP – по-английски
     MatchesMask(inputArchiveFileName, 'RCON_*_МТР*') or //MTP – по-русски
     MatchesMask(inputArchiveFileName, 'RCON_*_MTP*') then //MTP – по-английски
    InvoiceType := MTR_ACT;
  ...
  IF CreateProtocol(inputNotSigFile, inputSigFilesArray, DirectoryFrom, DirectoryToProcessed, DirectoryToInvoice, DirectoryToInvoiceMTR, DirectoryToAct, DirectoryToActMTR, inputArchiveFileName) = True THEN
    ...
    END
  ELSE //Если протокол не создался из-за особенной ошибки, перемещаем файлы в папку "TFOMS Errors" для рассмотрения Саше
    BEGIN
      if System.SysUtils.DirectoryExists(DirectoryTFOMSErrors) = False then
          System.SysUtils.ForceDirectories(DirectoryTFOMSErrors);

      //В папке "TFOMS Errors" создаём папку с именем оригинального zip-файла, присланного МО
      ...
      //Переносим файлы в папку "TFOMS Errors":
      ...
      MoveFile('E:\Proba\AutoProcessingFiles\SH_830082_83001_основной.zip', 'E:\Proba\AutoProcessingFiles\TFOMS Errors\SH_830082_83001_основной\SH_830082_83001_основной.zip');
...
begin
  ...
  RichEditLog.PlainText := True; //Всегда возвращаем значение на False для автоматических Логов в TXT-формате
end;
...
begin
  if SelectDirectory('Выберите папку для выгрузки Актов:', '', DirectoryAct, [sdNewFolder, sdShowShares, sdNewUI, sdValidateDir]) then
    EditActPath.Text := DirectoryAct;
end;

procedure TFormMain.ButtonActMTRpathClick(Sender: TObject);
begin
  if SelectDirectory('Выберите папку для выгрузки Актов-МТР:', '', DirectoryActMTR, [sdNewFolder, sdShowShares, sdNewUI, sdValidateDir]) then
